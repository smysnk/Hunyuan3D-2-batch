...
# Enable execution in Hunyuan3D-2GP/crawler
...
class ModelWorker:
  ...
  def generate(self, uid, params):
    ...
    if 'mesh' in params:
      ...
      seed = int(params.get("seed", random.randint(0, 10000000)))
      params['generator'] = torch.Generator(self.device).manual_seed(seed)
      params['octree_resolution'] = int(params.get("octree_resolution", 256))
      params['num_inference_steps'] = int(params.get("num_inference_steps", 40))
      params['guidance_scale'] = int(params.get('guidance_scale', 7))
      params['face_count'] = int(params.get('face_count', 40000))
      params['mc_algo'] = 'mc'
    ...
    if params.get('texture', False):
      ...
      mesh = FaceReducer()(mesh, max_facenum=params['face_count'])
...
async def generate(request: Request):
  ...
  uid = uuid.uuid4()
  try:
    file_path, uid = worker.generate(uid, { "image": image_bytes, **dict(request.query_params) })
